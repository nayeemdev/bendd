...
enum PerspectiveBend {
    static func matrix(angleDegrees: Float, aspectRatio: Float) -> float4x4 {
        let hingeRotation = rotationAroundBottomEdge(angleDegrees: angleDegrees)
        let projection = perspective(fovYRadians: .pi / 3, aspectRatio: aspectRatio, near: 0.1, far: 10)
        let view = translation(x: 0, y: 0, z: -3)
        return projection * view * hingeRotation
    }
    ...
    private static func rotationAroundBottomEdge(angleDegrees: Float) -> float4x4 {
        ...
        let rotation = rotationX(radians: radians)
        return moveHingeBack * rotation * moveHingeToOrigin
    }
